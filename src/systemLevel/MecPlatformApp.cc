...
void MecPlatformApp::processMecAppMigrationMessage(inet::Ptr<const MecAppMigrationMessage> message){
    ...
    disconnectApp(message->getContainerAppName());
    ...
    outmessage->setContainerAppName(message->getContainerAppName());
...
void MecPlatformApp::processMecAppTransfertMessage(inet::Ptr<const MecAppTransfertMessage> message){
    ...
    connectApp(message->getContainerAppName(),true);
...
void MecPlatformApp::createAppModule(string appName,string containerName,map<std::string, omnetpp::cParImpl *> parameter){
    cModuleType *moduleType = cModuleType::get(appName.c_str());
    auto sParentName = (containerName + ".vm");
    const char * parentName = sParentName.c_str();
...
void MecPlatformApp::disconnectApp(string containerName){
    const char * mecContainer = containerName.c_str();

    cModule *containerToMigrate = getModuleByPath(mecContainer);
    ...
    //Scollegala
    cGate *appGate_out = containerToMigrate->gate("vEth$o");  //portare fuori queste 2 var
    ...
    string macAddrApp = containerName + ".vm.eth[0]";
...
void MecPlatformApp::connectApp(string containerName,bool migration){
    ...
    string macAddrApp = containerName + ".vm.eth[0]";
    const char * macAddr = macAddrApp.c_str();
    const char * mecContainerApp = containerName.c_str();
    cModule *containerMigrating = getModuleByPath(mecContainerApp);
    inet::InterfaceEntry *ethApp = check_and_cast<inet::InterfaceEntry*>(getModuleByPath(macAddr));
    inet::MacAddress addr = ethApp->getMacAddress();



    cGate *appGate_out = containerMigrating->gate("vEth$o");
    cGate *appGate_in = containerMigrating->gate("vEth$i");
    ...
    route_label(containerName,migration);
}

void MecPlatformApp::route_label(string containerName,bool migration){
    ...
    if(migration == true){
        ...
        int id = readElementFromXml(par("fecFile"), containerName + ".vm", "id");
        int tunnel_id = readElementFromXml(par("fecFile"), containerName + ".vm", "tunnel_id");
        auto sdest = (containerName + ".vm");
    ...
    string mecAppRT = containerName + ".vm.ipv4.routingTable";
    const char * c1 = mecAppRT.c_str();
    inet::Ipv4RoutingTable *vm_routing_table = check_and_cast<inet::Ipv4RoutingTable*>(getModuleByPath(c1));
    ...
    std::string mecAppIT = containerName + ".vm.interfaceTable";
    const char * c2 = mecAppIT.c_str();
    ...
    //solo post migrazione
    std::string arp_module_string = containerName + ".vm.ipv4.arp";
